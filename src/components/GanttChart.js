...
import { getDaysBetween, formatDate } from '../holidays.js';
...
export const GanttChart = {
  ...
  setup(props, { emit }) {
    ...
    const activeDrag = Vue.ref(null);

    const tasks = Vue.computed(() => store.appData.projects[store.appData.currentProjectId]?.tasks || []);
    const validTasks = Vue.computed(() => tasks.value.filter(t => t.start && t.end));

    // 🔥 CPM 關鍵路徑計算
    const criticalTaskIds = Vue.computed(() => {
    ...
    const minDate = Vue.computed(() => props.customStart ? new Date(props.customStart) : (validTasks.value.length === 0 ? new Date() : new Date(Math.min(...validTasks.value.map(t => new Date(t.start))))));
    const maxDate = Vue.computed(() => props.customEnd ? new Date(props.customEnd) : (validTasks.value.length === 0 ? new Date() : new Date(Math.max(...validTasks.value.map(t => new Date(t.end))))));
    const totalDays = Vue.computed(() => Math.max(1, Math.round((maxDate.value - minDate.value) / (1000*60*60*24)) + 1));
    const labelWidth = Vue.computed(() => window.innerWidth < 768 ? 180 : 220);

    const monthSpans = Vue.computed(() => {
    ...
    const weekendIndexes = Vue.computed(() => {
    ...
    return {
      store, validTasks, totalDays, labelWidth, monthSpans, daysList, weekendIndexes, criticalTaskIds,
      hoverIndex, activeDrag, getBaselinePos, getDynamicBarPos, getStageColor, getStatusClass, getPreviewText, handleHover, startDrag
    };
  }
};
